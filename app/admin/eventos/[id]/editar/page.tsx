...
export default function EditarEventoPage() {
  ...
  useEffect(() => {
    ...
    async function loadEvent() {
      try {
        const { data: eventData, error } = await supabase.from('events').select('*').eq('id', eventId).single();
  ...
  const handleSlugFormat = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const formattedSlug = text.toLowerCase().trim().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-z0-9\s-]/g, "").replace(/\s+/g, "-");
  ...
  const handleUpdateEvent = async () => {
    if (!eventName) return showToast('Campo Requerido', 'Debes mantener el nombre del evento.', 'error');
    setIsSaving(true);

    try {
      if (eventSlug) {
        const { data: existingSlug } = await supabase.from('events').select('id').eq('slug', eventSlug).neq('id', eventId).single();
        if (existingSlug) throw new Error("El Alias para la URL ya pertenece a otro evento.");
      }
      ...
      const { error: eventError } = await supabase.from('events').update({
        ...
        send_notifications: sendNotifications, require_habeas_data: requireHabeasData,
      ...
      const fieldsToUpsert = fields.map((f, index) => {
        const isNew = String(f.id).startsWith('f-custom-');
        const payload: any = {
        ...
        <div className="lg:col-span-4 space-y-4">
          ...
          <AccordionSection id="detalles" icon={Settings} title="Detalles y Diseño Base" isOpen={openSection === 'detalles'} onToggle={toggleSection}>
            <div className="space-y-1.5">
              ...
              <input type="text" value={eventName} onChange={(e) => setEventName(e.target.value)} className="w-full bg-white dark:bg-black/50 border border-gray-300 dark:border-gray-700 rounded-lg py-3 px-4 text-gray-900 dark:text-white focus:border-accent outline-none" />
            </div>
            <div className="space-y-1.5">
              ...
              <div className="flex bg-white dark:bg-black/50 border border-gray-300 dark:border-gray-700 rounded-lg overflow-hidden group">
                  ...
                  <input type="text" value={eventSlug} onChange={handleSlugFormat} className="w-full bg-transparent py-3 px-4 text-gray-900 dark:text-white focus:outline-none" />
              </div>
            </div>
            <div className="space-y-1.5">
              ...
              <textarea value={eventDescription} onChange={(e) => setEventDescription(e.target.value)} rows={4} className="w-full bg-white dark:bg-black/50 border border-gray-300 dark:border-gray-700 rounded-lg py-3 px-4 text-gray-900 dark:text-white focus:border-accent outline-none resize-none" />
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <label className="text-sm text-gray-700 dark:text-gray-400 font-bold cursor-pointer">Logo Oficial</label>
                <div onClick={() => fileInputRef.current?.click()} className="w-full h-24 border-2 border-dashed border-gray-300 dark:border-gray-700 hover:border-primary bg-gray-50 dark:bg-black/30 rounded-xl flex flex-col items-center justify-center cursor-pointer relative overflow-hidden group">
                  {logoPreview ? <img src={logoPreview} alt="Preview" className="h-full w-full object-contain p-2" /> : <><ImageIcon className="h-6 w-6 text-gray-400 mb-1 group-hover:text-primary" /><span className="text-[10px] text-gray-500">Subir Logo</span></>}
              ...
              <div className="space-y-1.5">
                ...
                <div onClick={() => bannerInputRef.current?.click()} className="w-full h-24 border-2 border-dashed border-gray-300 dark:border-gray-700 hover:border-primary bg-gray-50 dark:bg-black/30 rounded-xl flex flex-col items-center justify-center cursor-pointer relative overflow-hidden group">
                  {bannerPreview ? <img src={bannerPreview} alt="Preview" className="h-full w-full object-cover" /> : <><ImagePlus className="h-6 w-6 text-gray-400 mb-1 group-hover:text-primary" /><span className="text-[10px] text-gray-500">Subir Portada</span></>}
          ...
          <AccordionSection id="seguridad" icon={ShieldAlert} title="Acceso y Restricciones" isOpen={openSection === 'seguridad'} onToggle={toggleSection}>
            ...
            <div className="space-y-1.5">
              ...
              <input type="number" value={maxCapacity} onChange={(e) => setMaxCapacity(e.target.value)} placeholder="Vacio para ilimitado" className="w-full bg-white dark:bg-black/50 border border-gray-300 dark:border-gray-700 rounded-lg py-3 px-4 text-gray-900 dark:text-white focus:border-accent outline-none" />
            </div>
            <div className="space-y-1.5">
              ...
              <input type="datetime-local" value={closeDate} onChange={(e) => setCloseDate(e.target.value)} className="w-full bg-white dark:bg-black/50 border border-gray-300 dark:border-gray-700 rounded-lg py-3 px-4 text-gray-900 dark:text-white focus:border-accent outline-none" />
            </div>
          </AccordionSection>
          ...
          <AccordionSection id="comunicaciones" icon={Mail} title="Comunicaciones y Mensajes" isOpen={openSection === 'comunicaciones'} onToggle={toggleSection}>
            <div className="flex items-center justify-between bg-gray-50 dark:bg-black/30 p-4 rounded-xl border border-gray-200 dark:border-gray-800">
              <div>
                  ...
                  <p className="text-xs text-gray-500 dark:text-gray-400">Envía un código QR automático.</p>
              </div>
              <button type="button" role="switch" aria-checked={sendNotifications} onClick={() => setSendNotifications(!sendNotifications)} className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${sendNotifications ? 'bg-blue-500' : 'bg-gray-300 dark:bg-gray-700'}`}>
            ...
            <div className="space-y-4 pt-4 border-t border-gray-200 dark:border-white/5">
              <div className="flex items-center justify-between">
                ...
                <button type="button" role="switch" aria-checked={thankYouEnabled} onClick={() => setThankYouEnabled(!thankYouEnabled)} className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${thankYouEnabled ? 'bg-accent' : 'bg-gray-300 dark:bg-gray-700'}`}>
                ...
                {thankYouEnabled && (
                  <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="space-y-4 overflow-hidden">
                    <div className="space-y-1.5">
                      ...
                      <textarea value={thankYouText} onChange={(e) => setThankYouText(e.target.value)} rows={3} className="w-full bg-white dark:bg-black/50 border border-gray-300 dark:border-gray-700 rounded-lg py-2.5 px-4 text-sm text-gray-900 dark:text-white focus:border-accent outline-none resize-none" />
                    </div>
                    <div className="space-y-1.5">
                      <label className="text-xs text-gray-600 dark:text-gray-400 font-bold">Redirección Automática (URL)</label>
                      <input type="url" value={thankYouUrl} onChange={(e) => setThankYouUrl(e.target.value)} className="w-full bg-white dark:bg-black/50 border border-gray-300 dark:border-gray-700 rounded-lg py-2.5 px-4 text-sm text-gray-900 dark:text-white focus:border-accent outline-none" />
                    </div>
                  </motion.div>
                )}
              </AnimatePresence>
          ...
          <AccordionSection id="legal" icon={ShieldCheck} title="Políticas Legal e Identidad" isOpen={openSection === 'legal'} onToggle={toggleSection}>
            <div className="flex items-center justify-between bg-gray-50 dark:bg-black/30 p-4 rounded-xl border border-gray-200 dark:border-gray-800">
              ...
              <button type="button" role="switch" aria-checked={requireHabeasData} onClick={() => setRequireHabeasData(!requireHabeasData)} className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${requireHabeasData ? 'bg-green-500' : 'bg-gray-300 dark:bg-gray-700'}`}>
            ...
            <AnimatePresence initial={false}>
              {requireHabeasData && (
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="space-y-1.5 overflow-hidden">
                    ...
                    <input type="url" value={habeasDataUrl} onChange={(e) => setHabeasDataUrl(e.target.value)} className="w-full bg-white dark:bg-black/50 border border-gray-300 dark:border-gray-700 rounded-lg py-2.5 px-4 text-xs text-gray-900 dark:text-white focus:border-accent outline-none" />
                </motion.div>
              )}
            </AnimatePresence>
          </AccordionSection>
        ...
        <div className="lg:col-span-8">
          <div className="bg-white dark:bg-surface border border-gray-200 dark:border-white/5 p-6 md:p-8 rounded-2xl shadow-sm dark:shadow-none">
            ...
            <div className="space-y-4">
                      ...
                      {field._ui_showLogic && (
                        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-800 bg-primary/5 -mx-5 px-5 pb-2 rounded-b-xl overflow-hidden">
                          <div className="flex items-center justify-between mb-3">
                            <h4 className="text-sm font-bold text-primary flex items-center gap-2"><GitBranch className="h-4 w-4" /> Lógica Condicional</h4>
                            {field.logic && <button type="button" onClick={() => clearFieldLogic(field.id)} className="text-xs text-red-500 hover:text-red-600 cursor-pointer">Quitar Lógica</button>}
                          </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                              <div>
                                ...
                                <select value={field.logic?.action || 'show'} onChange={(e) => updateFieldLogic(field.id, 'action', e.target.value)} className="w-full bg-white dark:bg-black/50 border border-gray-300 dark:border-gray-700 text-sm text-gray-900 dark:text-white rounded-lg p-2 focus:border-primary outline-none cursor-pointer">
                                  <option value="show">Mostrar la pregunta si...</option><option value="hide">Ocultar la pregunta si...</option><option value="require">Hacer Obligatoria si...</option>
                                </select>
                              </div>
                              <div>
                                ...
                                <select value={field.logic?.dependsOnId || ''} onChange={(e) => updateFieldLogic(field.id, 'dependsOnId', e.target.value)} className="w-full bg-white dark:bg-black/50 border border-gray-300 dark:border-gray-700 text-sm text-gray-900 dark:text-white rounded-lg p-2 focus:border-primary outline-none cursor-pointer">
                                  ...
                                  {availableParentQuestions.map(p => <option key={p.id} value={p.id}>{p.label}</option>)}
                                </select>
                              </div>
                              <div>
                                ...
                                <select value={field.logic?.dependsOnValue || ''} onChange={(e) => updateFieldLogic(field.id, 'dependsOnValue', e.target.value)} disabled={!field.logic?.dependsOnId} className="w-full bg-white dark:bg-black/50 border border-gray-300 dark:border-gray-700 text-sm text-gray-900 dark:text-white rounded-lg p-2 disabled:opacity-50 focus:border-primary outline-none cursor-pointer">
                                  ...
                                  {selectedParent?.options.map(opt => <option key={opt} value={opt}>{opt}</option>)}
                                </select>
                              </div>
                            </div>
                          )}
              ...
              <button type="button" onClick={handleAddField} className="w-full mt-6 border-2 border-dashed border-gray-300 dark:border-gray-700 hover:border-accent bg-transparent text-gray-500 hover:text-accent font-bold py-4 rounded-xl flex items-center justify-center gap-2 transition-colors cursor-pointer">
